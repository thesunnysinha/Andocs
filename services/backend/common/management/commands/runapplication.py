...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options['celery']:
            ...
            celery_worker_command = ['celery', '-A', 'Andocs', 'worker', '--loglevel=info']
            try:
        ...
        if options['celery_beat']:
            ...
            celery_beat_command = ['celery', '-A', 'Andocs', 'beat', '--loglevel=info']
            try:
        ...
        gunicorn_command = ['gunicorn', 'Andocs.wsgi:application', '--config', '/app/gunicorn.prod.conf.py', '--reload']
        try:
